...
if [ -e /usr/local/cpanel/cpanel ]; then
	if [ "$1" = "d" ]; then
		find /home/*/logs -maxdepth 1 -type f | grep -v ssl | grep -v ftp | grep -v byte | grep -v siteupda | xargs zgrep $querydate 2>/dev/null > $logs
	else
		find /usr/local/apache/domlogs -maxdepth 1 -type f | grep -v ssl | grep -v ftp | grep -v byte | grep -v siteupda | xargs grep $querydate 2>/dev/null > $logs
                ...
                find /home/*/access_logs -type f | grep -v ftp | xargs zgrep $querydate 2>/dev/null >> $logs
	fi
elif [ -e /usr/local/psa/version ]; then
        if [ "$1" = "d" ]; then
		if [ -e /var/www/vhosts/system ]; then
			find /var/www/vhosts/system -name access_log* | xargs zgrep $querydate 2>/dev/null > $logs
		else
			find /var/www/vhosts -name access_log* | xargs zgrep $querydate 2>/dev/null > $logs
		fi
	else
                if [ -e /var/www/vhosts/system ]; then
                        find /var/www/vhosts/system -name "*access_log" | xargs grep $querydate 2>/dev/null  > $logs
                else
                        find /var/www/vhosts -name "*access_log" | xargs grep $querydate 2>/dev/null > $logs
                fi
	fi
else
	find /var/log -name "*access_log*" -o -name "*access.log*" -o -name "*access_ssl_log*" | xargs zgrep $querydate 2>/dev/null > $logs
